...
SELECT_VOTE_INFORMATION = """SELECT options.id, 
    options.option_text, 
...
INSERT_POLL_WITH_ID_RETURN = "INSERT INTO polls (title, owner_username) VALUES (%s, %s) RETURNING id;"
...
def get_poll_and_vote_results(connection, poll_id):
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(SELECT_VOTE_INFORMATION, (poll_id,))
            return cursor.fetchall()
...
def create_poll(connection, title, owner, options):
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(INSERT_POLL_WITH_ID_RETURN, (title, owner))

            poll_id = cursor.fetchone()[0]
            option_values = [(option_text, poll_id) for option_text in options]

            execute_values(cursor, INSERT_OPTION, option_values)  # same as loop below
